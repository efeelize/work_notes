...
> $:消费者案件 only 中院审理
       usu 侵权行为地、被告住所地 + 中
       exp 环境 + 中认为有必要 + 高批 + 交 基院
           环境、消费 + 最高批 + 高院确定中院（depends本辖区环境&&生态保护的实际情况）；
...
###    3)调解书、调解协议 一裁终局
         exp:检院有限抗诉( only 前提损害国\社利益)

## 1.4 撤诉
###    1）if 在法庭辩论终结 前 撤诉；准予撤诉
###    2）else if 在法辩终结 后 撤诉；不准撤诉
...
###  2）检院 
          if 1）的主体起诉；
                   ...
                   检院 提公益诉讼
###  3)消协最低省级
          环境污染组织：设区的市级以上民政部门登记 + 连续5years + 无违法记录
###  4）追加 P
          其他组织 + 开庭前 + 共同P参诉
###  5）执行
          if 公益诉讼案件裁、判生效 + D 不履行；
                    移送执行
